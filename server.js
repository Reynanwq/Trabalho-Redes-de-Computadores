...
function recover(client, clientName, filename) {
    ...
    const directories = fs.readdirSync(DIRECTORYSERVER);

    //itera sobre os diretórios para cada copia
    directories.forEach((folder) => {
        const folderPath = path.join(DIRECTORYSERVER, folder);
    ...
    if (!fileFound) {
        socket.emit(`[WARNING] File ${filename} not found\n`);
    }
}
