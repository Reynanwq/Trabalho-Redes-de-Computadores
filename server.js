...
io.on('connection', (socket) => {
    ...
    socket.on('command', (data) => {
        ...
        if (command === 'list') {
            ...
            else socket.write("Please write all arguments!")
        } else if (command === 'delete') {
            if (args[2]) deleteFile(socket, args[0], args[1]);
            else socket.write("Please write all arguments!");
...
-----------------------------------------------------------------
*/
function recover(client, stream, data) {
    const { clientName, filename } = data;
    let fileFound = false;
    ...
    directories.forEach((folder) => {
        ...
        // verifica se o diretório do cliente existe dentro do destino de cópia
        if (folderFiles.includes(clientName)) {
            ...
            if (clientFiles.includes(filename)) {
                ...
                // Usando o socket.io-stream para enviar o arquivo ao cliente
                ss(client).emit('recoverfile', stream, { clientName, filename });
                fileReadStream.pipe(stream);

                fileReadStream.on('end', () => {
                    // Mensagem indicando que o arquivo foi recuperado com sucesso
    ...
    if (!fileFound) {
        client.emit('message', `[WARNING] File ${filename} not found`);
    }
}
